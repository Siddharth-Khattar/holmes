...
must_haves:
  truths:
    ...
    - "JWT tokens are issued on sign-in and retrievable via /api/auth/token"
...
export const auth = betterAuth({
  ...
  plugins: [
    jwt({
      issueTokensOnSignIn: true, // CRITICAL: Actually issue JWT tokens that client can retrieve
    }),
...
```
  ...
  <verify>
    ...
    # Also verify JWT token endpoint works (after starting dev server):
    # curl -X POST http://localhost:3000/api/auth/sign-in/email -d '{"email":"test@test.com","password":"password123"}' -H "Content-Type: application/json"
    # curl http://localhost:3000/api/auth/token -H "Cookie: <session_cookie>"
  </verify>
  <done>Better Auth configured with JWT plugin (issueTokensOnSignIn: true), client with token retrieval, and API client with Authorization header. Token endpoint /api/auth/token returns valid JWT.</done>
</task>
...
7. JWT plugin configured with `issueTokensOnSignIn: true`
</verification>

<success_criteria>
- Better Auth handles /api/auth/* endpoints (signup, signin, signout, jwks, token, etc.)
- JWT plugin enabled with issueTokensOnSignIn: true - tokens actually issued on login
- JWKS endpoint available at /api/auth/jwks
- Token endpoint /api/auth/token returns valid JWT for authenticated users
