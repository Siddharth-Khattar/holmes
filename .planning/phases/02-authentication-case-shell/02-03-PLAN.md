...
<objective>
Set up Better Auth in Next.js with email/password and Google OAuth providers.

Purpose: Enable user authentication with session persistence in PostgreSQL. Creates the auth foundation that the backend reads from.
Output: Better Auth configuration, API routes, client hooks, and route protection middleware.
</objective>
...
<task type="auto">
  <name>Task 2: Create Better Auth server configuration</name>
  <files>
    frontend/src/lib/auth.ts
    frontend/src/lib/auth-client.ts
    frontend/src/app/api/auth/[...all]/route.ts
  ...
  <action>
1. Create `frontend/src/lib/auth.ts` (server-side config):

```typescript
// ABOUTME: Better Auth server configuration for Next.js
// ABOUTME: Handles email/password, Google OAuth, and session management
...
2. Create `frontend/src/lib/auth-client.ts` (client-side hooks):

```typescript
// ABOUTME: Better Auth client for React components
// ABOUTME: Provides hooks and functions for auth operations
...
3. Create `frontend/src/app/api/auth/[...all]/route.ts`:

```typescript
// ABOUTME: Better Auth API route handler
// ABOUTME: Catches all /api/auth/* requests
...
```
  ...
  <done>Better Auth configured with email/password and Google OAuth</done>
</task>
...
<success_criteria>
- Better Auth handles /api/auth/* endpoints (signup, signin, signout, etc.)
...
- Client hooks (signIn, signUp, signOut, useSession) exported
</success_criteria>
