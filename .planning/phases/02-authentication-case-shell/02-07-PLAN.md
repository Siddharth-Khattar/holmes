...
must_haves:
  ...
  artifacts:
    ...
    - path: "terraform/iam.tf"
      provides: "Frontend Cloud SQL client role"
      contains: "frontend_cloudsql_client"
...
<task type="auto">
  <name>Task 2: Update Cloud Run services with database, secrets, and cross-service URLs</name>
  <files>
    terraform/cloud-run.tf
    terraform/iam.tf
  </files>
  <action>
Update terraform/cloud-run.tf to replace the COMPLETE frontend and backend service resources. Also add frontend Cloud SQL client role to terraform/iam.tf.

**COMPLETE terraform/cloud-run.tf replacement:**
...
**APPEND to terraform/iam.tf (after existing content):**
...
```
  ...
  <verify>
    cd /Users/siddharth/Development/Hackathons/Gemini_3_2025/holmes && grep -A5 "BETTER_AUTH_SECRET" terraform/cloud-run.tf && grep "volume_mounts" terraform/cloud-run.tf | head -2 && grep "FRONTEND_URL" terraform/cloud-run.tf && grep "frontend_cloudsql_client" terraform/iam.tf && echo "Cloud Run and IAM updated"
  </verify>
  <done>Frontend Cloud Run service updated with Cloud SQL access and secrets. Backend has FRONTEND_URL. IAM has frontend Cloud SQL client role.</done>
</task>
...
2. terraform/cloud-run.tf has frontend with Cloud SQL mount and secrets (complete resource block)
3. terraform/cloud-run.tf has backend with FRONTEND_URL env var (complete resource block)
4. terraform/iam.tf has frontend Cloud SQL client role
5. .github/workflows/deploy.yml runs migrations and sets auth env vars
6. terraform fmt passes: `cd terraform && terraform fmt -check`
</verification>
...
- Backend Cloud Run has FRONTEND_URL env var for JWKS endpoint
- terraform/iam.tf grants frontend Cloud SQL client role
- CI/CD runs Alembic migrations after backend deploy
