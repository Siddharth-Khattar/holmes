...
3. **Key implementation details**:
   - Use `settings.gemini_pro_model` (MODEL_PRO) for complex reasoning per CONTEXT.md
   ...
   - **IMPORTANT: Orchestrator runs in a FRESH stage-isolated session**
   - **Input is TEXT ONLY**: TriageOutput JSON from database (no file content!)
   - This keeps Orchestrator's context lightweight (~10-50K tokens vs 500K+ with files)
...
- Use `settings.gemini_pro_model` instead of hardcoded model ID
- Use `planner=create_thinking_planner("high")` for thinking config
- Add `output_key="orchestrator_result"` for state passing pattern
- Optional: Consider AgentTool(triage_agent) pattern if Orchestrator needs to re-invoke Triage
