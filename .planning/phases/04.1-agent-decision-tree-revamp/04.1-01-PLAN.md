...
<task type="auto">
  ...
  **Step 2: Add scoped Command Center CSS variables to `globals.css`.**
  ...
  - `@xyflow/react` and `@dagrejs/dagre` appear in `package.json` (no `@types/dagre` needed — v2.0.3 ships own types)
...
<verification>
- Both new npm packages installed: `@xyflow/react`, `@dagrejs/dagre` (v2.0.3 ships own TS types)
