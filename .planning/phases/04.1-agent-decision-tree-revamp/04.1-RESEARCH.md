...
**Primary recommendation:** Install `@xyflow/react`, `@dagrejs/dagre`, and `@types/dagre`. Rewrite `AgentFlowCanvas.tsx` as the main React Flow container, `AgentNode.tsx` as a custom `DecisionNode`, and `AgentDetailsPanel.tsx` as a spring-animated sidebar. Add scoped teal CSS variables in `globals.css`. Wrap in `ReactFlowProvider` at the `CommandCenter.tsx` level.
...
| `@dagrejs/dagre` | 1.1.8 | Automatic hierarchical (TB) graph layout | Actively maintained fork of dagre; official React Flow docs use it for tree layouts |
...
|---------|---------|---------|-------------|
| `@types/dagre` | latest | TypeScript definitions for dagre | Always -- needed for typed dagre API access |
...
- **Using `dagre` (unmaintained v0.8.5):** Use `@dagrejs/dagre` v1.1.8 instead.
...
**Dagre maintainer change:** `dagre` (v0.8.5, unmaintained) was forked to `@dagrejs/dagre` (v1.1.8, active). Always use `@dagrejs/dagre`.
...
- `@dagrejs/dagre` v1.1.8 -- [npm](https://www.npmjs.com/package/@dagrejs/dagre)
