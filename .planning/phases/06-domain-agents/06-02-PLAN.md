...
must_haves:
  truths:
    ...
    - "All prompts include a note about receiving optional case-specific context injection"
  artifacts:
...
<objective>
Create domain-specific system prompts for all four agents: Financial, Legal, Evidence, and Legal Strategy. Each prompt defines the agent's analysis scope, entity taxonomy, finding categories, hypothesis evaluation instructions, context injection awareness, and output JSON schema.

Purpose: Prompts are the core intelligence of each agent. They determine extraction quality, entity coverage, and finding structure. Prompt modules are separate from agent logic per the established pattern (see triage.py, orchestrator.py prompts). Each prompt must include awareness that it may receive case-specific context injection prepended to the analysis section.
...
10. **Context Injection Awareness:**
    - Add this note early in the prompt: "You may receive additional case-specific context at the start of the analysis section. This context is provided by the Orchestrator to help you focus your analysis on the most relevant aspects of the case. Use it to guide your analysis priorities, but still apply your full financial analysis expertise."
...
12. **Output format:** Raw JSON matching FinancialOutput schema. Same "no code fences, no commentary" instruction as triage/orchestrator prompts.
...
6. **Context Injection Awareness:** Same note as Financial -- "You may receive additional case-specific context at the start of the analysis section. Use it to focus your analysis appropriately."

7. **Additional legal-specific instructions:**
...
4. `python -c "from app.agents.prompts.financial import FINANCIAL_SYSTEM_PROMPT; assert 'case-specific context' in FINANCIAL_SYSTEM_PROMPT; print('context injection note found')"` -- verify context injection awareness.
  </verify>
  <done>
Financial and Legal system prompts created with domain-specific entity taxonomies, finding categories, hypothesis evaluation instructions, confidence scoring guidance, context injection awareness note, and structured JSON output format matching their respective Pydantic schemas.
  </done>
</task>
...
6. **Context Injection Awareness:** Same note as Financial/Legal -- "You may receive additional case-specific context at the start of the analysis section. Use it to focus your analysis appropriately."

7. **Evidence-specific constraints:**
...
8. **Quality Assessment output:** Instruct the model to always produce the `quality_assessment` object with `overall_score`, `authenticity_concerns`, `custody_chain_complete`, `custody_gaps`, `corroboration_status`, and `recommendation`.
...
6. **Context Injection Awareness:** Same note as other agents -- "You may receive additional case-specific context at the start of the analysis section. Use it to focus your analysis appropriately."

7. **Strategy-specific constraints:**
...
8. **Additional field**: `domain_agent_summaries_received` -- list of agent names whose summaries were included in the context. The prompt should instruct: "List which domain agent summaries you received in the domain_agent_summaries_received field."

9. Same hypothesis evaluation, confidence scoring, citation requirements, no-findings handling pattern. Strategy agent citations point to its own files (playbooks, strategy docs), not to files other agents processed.
  </action>
  <verify>
Run from the backend directory:
...
5. `python -c "from app.agents.prompts.evidence import EVIDENCE_SYSTEM_PROMPT; assert 'case-specific context' in EVIDENCE_SYSTEM_PROMPT; print('context injection note found')"` -- verify context injection awareness.
  </verify>
  <done>
...
- Each prompt includes note about receiving optional case-specific context injection
...
- All prompts include context injection awareness note
</success_criteria>
