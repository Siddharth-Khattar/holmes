...
<task type="auto">
  <name>Task 1: Financial agent module (template for other agents)</name>
  <files>
    backend/app/agents/financial.py
  </files>
  <action>
...
2. `python -c "import inspect; from app.agents.financial import run_financial; sig = inspect.signature(run_financial); assert 'hypotheses' in sig.parameters; print('hypotheses param exists')"` -- verify signature.
  </verify>
  <done>
Financial agent module created with: Agent wrapper class, output parser, content preparer using build_domain_agent_content, and run function with inline Pro-to-Flash fallback (REQ-AGENT-007h), hypothesis context injection, stage-isolated sessions, and execution logging.
  </done>
</task>

<task type="auto">
  <name>Task 2: Legal and Evidence agent modules + parallel runner</name>
  <files>
    backend/app/agents/legal.py
    backend/app/agents/evidence.py
    backend/app/agents/domain_runner.py
...
- legal.py: agent_name="legal", LegalAgent, LegalOutput, parse_legal_output, run_legal, stage="legal", stage_fallback="legal_fallback"
...
- evidence.py: agent_name="evidence", EvidenceAgent, EvidenceOutput, parse_evidence_output, run_evidence, stage="evidence", stage_fallback="evidence_fallback"
...
All three share the same inline Pro-to-Flash fallback pattern (REQ-AGENT-007h).

**2b. Create `backend/app/agents/domain_runner.py`:**

The parallel execution coordinator for Financial, Legal, and Evidence agents.
...
3. `python -c "from app.agents.domain_runner import run_domain_agents_parallel, build_strategy_context; print('domain_runner imports OK')"` -- imports work.
...
5. `python -c "from app.agents.domain_runner import build_strategy_context; result = build_strategy_context({}); print(f'Empty context: {repr(result)}')"` -- works with empty input.
  ...
  <done>
Legal and Evidence agent modules created following financial.py template (same inline Pro-to-Flash fallback for REQ-AGENT-007h). domain_runner.py created with:
