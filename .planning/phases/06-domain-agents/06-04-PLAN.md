...
must_haves:
  ...
  artifacts:
    - path: "backend/app/agents/strategy.py"
      provides: "StrategyAgent class + run_strategy function"
...
<objective>
Implement the Legal Strategy agent that runs AFTER Financial, Legal, and Evidence agents. It receives its own strategy-relevant files plus text summaries of other agents' findings. It does not re-analyze raw evidence -- it synthesizes a strategic picture.
...
async def _prepare_strategy_content(
    ...
    prompt_parts: list[str] = [
        "Analyze the following for legal strategy insights.",
    ]
...
**run_strategy function signature -- DIFFERENT from other agents:**
...
- Stage name is "strategy" (and "strategy_fallback" for Flash fallback)
- agent_name="strategy" in execution record
- Content preparation uses `_prepare_strategy_content` with domain_summaries
...
2. `python -c "import inspect; from app.agents.strategy import run_strategy; sig = inspect.signature(run_strategy); assert 'domain_summaries' in sig.parameters; print('domain_summaries param exists')"` -- verify unique parameter.
...
- _prepare_strategy_content that combines strategy files + domain agent text summaries + hypothesis context
- run_strategy with domain_summaries parameter, Pro-to-Flash fallback, stage-isolated session
...
<verification>
- Strategy agent importable and has correct unique signature (domain_summaries param)
...
- Stage name "strategy" and "strategy_fallback" used for session isolation
</verification>

<success_criteria>
- `run_strategy` accepts domain_summaries parameter (text from build_strategy_context)
