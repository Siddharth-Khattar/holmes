...
must_haves:
  truths:
    - "Full pipeline runs: Triage -> Orchestrator -> Parallel Domain Agents (file-group-based) -> Strategy -> processing-complete"
    - "SSE events use compound identifiers ({agent_type}_{group_label}) for multiple instances of same agent type"
    - "Each domain agent instance emits agent-started and agent-complete SSE events with metadata"
    - "Low-confidence findings trigger HITL confirmation via the existing confirmation service"
    - "Pipeline status endpoint reflects domain_analysis stage and handles multiple executions per agent type"
    ...
    - "Results dict from domain_runner uses new multi-result structure (list per agent type)"
    - "SSE pre-emission uses compute_agent_tasks from domain_runner (no duplicated iteration logic)"
  artifacts:
    - path: "backend/app/api/agents.py"
      provides: "Extended run_analysis_workflow with domain agent stages 3, 4, and 5; uses compute_agent_tasks for SSE pre-emission"
  ...
  key_links:
    - from: "backend/app/api/agents.py"
      ...
      via: "import and call run_domain_agents_parallel and compute_agent_tasks"
      pattern: "from app.agents.domain_runner import run_domain_agents_parallel, compute_agent_tasks"
    - from: "backend/app/api/agents.py"
...
<objective>
Wire the domain agents into the analysis pipeline, add SSE event emission for domain agent lifecycle (using compound identifiers for multiple instances of the same agent type), integrate HITL confirmation for low-confidence findings, and update the pipeline status endpoint to handle multiple executions per agent type. This completes the Phase 6 backend implementation.

Purpose: This is the integration plan that connects all the pieces built in Plans 01-04 into the working pipeline. The key complexity is handling the new file-group-based spawning: `run_domain_agents_parallel` returns `dict[str, list[tuple[BaseModel | None, str]]]` where each agent type can have multiple results (one per file group). SSE events, status tracking, and the strategy context builder must all work with this multi-result structure. SSE pre-emission uses `compute_agent_tasks` from `domain_runner.py` to avoid duplicating file-group iteration logic.

Output: Extended `agents.py` pipeline with stages 3-5 (domain + strategy + HITL), SSE events for all domain agent instances, updated status endpoint.
</objective>
...
<task type="auto">
  <name>Task 1: Add emit_agent_fallback helper and wire Stage 3 (file-group-based parallel domain agents)</name>
  <files>
    backend/app/services/agent_events.py
    backend/app/api/agents.py
  </files>
...
```python
from app.agents.domain_runner import run_domain_agents_parallel, build_strategy_context, compute_agent_tasks
...
Note: `_get_sessionmaker` is ALREADY imported on line 176 and `session_factory` is ALREADY created on line 179. The domain_runner's `db_session_factory` parameter will receive this same `session_factory` variable.

**1c. Insert Stage 3 (File-Group-Based Parallel Domain Agents) into `run_analysis_workflow`:**
...
The key change from the original plan: SSE events use `compute_agent_tasks` from `domain_runner.py` instead of re-implementing the file-group iteration logic. This ensures the SSE pre-emission always matches exactly the agent tasks that `run_domain_agents_parallel` will actually execute.
...
if orchestrator_output:
    ...
    for task in expected_tasks:
        ...
        await emit_agent_started(
            case_id=case_id,
            agent_type=compound_id,  # Use compound identifier
            task_id=task_id,
    ...
    for agent_type, result_list in domain_results.items():
        for result, group_label in result_list:
            ...
            if result is not None:
                ...
                # Query execution record for metadata (match on agent_name + stage suffix)
...
4. Verify by code inspection: `compute_agent_tasks` is imported from `domain_runner` and used for SSE pre-emission -- no duplicated file-group iteration logic in agents.py.
  </verify>
  <done>
- emit_agent_fallback helper added to agent_events.py
- Stage 3 (file-group-based parallel domain agents) wired into pipeline after orchestrator
- SSE pre-emission uses compute_agent_tasks from domain_runner.py (single source of truth, no duplicated iteration logic)
- SSE events use compound identifiers ({agent_type}_{group_label}) for multiple instances of same agent type
- session_factory (already `_get_sessionmaker()` on line 179) passed to run_domain_agents_parallel
- domain_results type is dict[str, list[tuple[BaseModel | None, str]]] matching domain_runner return type
...
Strategy agent consumes the new multi-result domain_results structure via `build_strategy_context`.
...
HITL must iterate over the new multi-result structure (list of tuples per agent type):
        ...
        for result, group_label in result_list:
            ...
            for finding in result.findings:
                if finding.confidence < CONFIDENCE_THRESHOLD:
                    logger.info(
                        "Low-confidence finding from %s (%s): %s (confidence=%s), requesting HITL",
                        agent_type, group_label, finding.title, finding.confidence,
                    )
                    ...
                    if not confirmation_result.approved:
                        logger.info(
                            "Finding rejected by user: %s from %s/%s (reason: %s)",
                            finding.title, agent_type, group_label, confirmation_result.reason,
                        )
                        # Mark finding as rejected (for audit trail)
                        # Finding remains in output but will be excluded from KG in Phase 7

# Also check strategy results
if strategy_result and hasattr(strategy_result, 'findings'):
    for finding in strategy_result.findings:
        if finding.confidence < CONFIDENCE_THRESHOLD:
            logger.info(
                "Low-confidence finding from strategy: %s (confidence=%s), requesting HITL",
            ...
            if not confirmation_result.approved:
                logger.info(
                    "Finding rejected by user: %s from strategy (reason: %s)",
                    finding.title, confirmation_result.reason,
                )
```
...
4. Code review: confirm multi-result iteration (for agent_type, result_list in domain_results.items(): for result, group_label in result_list:) is used consistently in HITL and processing-complete.
  </verify>
  <done>
- Stage 4 (Strategy agent, sequential after parallel agents) wired in with context_injection support
- Stage 5 (HITL confirmation for low-confidence findings) wired in, iterates multi-result structure correctly
...
- Finding/entity counts from multi-result domain agents included in processing-complete event
- Strategy findings also checked for HITL
  </done>
</task>

<task type="auto">
  <name>Task 3: Update status endpoint and response schema for multi-instance domain agents</name>
  <files>
    backend/app/api/agents.py
  </files>
  <action>
...
domain_results_summary: dict[str, list[dict[str, object]]] | None = Field(
    default=None,
    description="Summary of domain agent findings per agent type. "
...
Note: Changed from `dict[str, int]` to `dict[str, list[dict[str, object]]]` to support multiple executions per agent type. Each entry looks like:
```json
{
  "financial": [
    {"group_label": "grp_0", "finding_count": 5, "status": "completed"},
    {"group_label": "grp_1", "finding_count": 3, "status": "completed"}
  ],
  "legal": [
    {"group_label": "ungrouped_0", "finding_count": 2, "status": "completed"}
  ]
}
```
...
```python
domain_execs: dict[str, list[AgentExecution]] = {}  # agent_name -> list of executions
strategy_exec = None
...
Build domain_results_summary from domain execution records (multi-execution-aware):
```python
    ...
    for agent_name, exec_list in domain_execs.items():
        ...
        for exec_record in exec_list:
            ...
            if exec_record.input_data and isinstance(exec_record.input_data, dict):
                group_label = exec_record.input_data.get("stage_suffix", "").lstrip("_") or "default"
            ...
            domain_summary[agent_name].append({
                ...
                "status": exec_record.status.value if exec_record.status else "unknown",
            })
```
...
3. Review that the pipeline stages are: Triage -> Orchestrator -> File-Group-Based Parallel Domain -> Strategy -> HITL -> Status Update -> Processing Complete.
  </verify>
  <done>
- AnalysisStatusResponse extended with domain_results_summary field (supports multiple executions per agent type)
...
- Multiple executions per agent type tracked with group_label and finding_count per execution
- Backward compatible: orchestrator-complete without domain agents still shows "complete"
- Pipeline docstrings updated to reflect full 8-step pipeline with file-group-based domain agents
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
Complete Phase 6 domain agent pipeline with file-group-based spawning:
- 4 domain agents (Financial, Legal, Evidence, Strategy) with Pydantic schemas, prompts, execution functions
- File-group-based parallel execution: one agent instance per (file_group, agent_type) pair
- Context injection from orchestrator (RoutingDecision.context_injection for individual files, FileGroupForProcessing.shared_context for groups)
- Compound SSE identifiers ({agent_type}_{group_label}) for multiple instances of same agent type
- Pro-to-Flash fallback with SSE warning events
- HITL confirmation for low-confidence findings
- Full pipeline integration (Triage -> Orchestrator -> File-Group Domain -> Strategy -> HITL -> Complete)
- SSE pre-emission uses compute_agent_tasks (no duplicated iteration logic)
  </what-built>
  <how-to-verify>
1. Start the backend: `cd backend && python -m uvicorn app.main:app --reload`
2. Upload 3-4 evidence files to a case via the frontend Case Library (mix of financial docs, legal docs, evidence images)
...
5. Verify in the Command Center:
   ...
   - Domain agent nodes appear with compound identifiers (e.g., financial_grp_0, legal_ungrouped_0)
   - If orchestrator groups files, multiple nodes for same agent type should appear
   - Strategy node appears after domain agents complete
   - Thinking traces stream in real-time for each agent instance
...
6. Check backend logs for:
   ...
   - Group labels in log messages (grp_0, ungrouped_0, etc.)
   - Any HITL confirmation triggers (if findings have low confidence)
7. Check analysis status endpoint: `curl http://localhost:8000/api/cases/{case_id}/analysis/{workflow_id}` -- should show domain_results_summary with per-group execution summaries
...
<verification>
- Pipeline runs end-to-end: Triage -> Orchestrator -> File-Group Domain Agents -> Strategy -> Complete
- SSE events use compound identifiers for multiple instances of same agent type
- SSE pre-emission uses compute_agent_tasks from domain_runner (no duplicated iteration logic)
- Multiple file groups for same domain spawn concurrent agent instances
- Ungrouped files handled as implicit single-file groups
- Context injection flows from orchestrator routing decisions / file groups through to agent prompts
- Low-confidence findings trigger HITL confirmation modal
- Fallback events appear when Pro model fails
- Pipeline status endpoint shows correct stage with multi-execution awareness
...
<success_criteria>
- Upload files, start analysis, and see agent instances execute with group-specific identifiers in Command Center
- Each agent instance shows in the decision tree with thinking traces, token counts, and timing
- If orchestrator creates file groups, multiple instances of same agent type run concurrently
- Strategy agent runs AFTER parallel agents and incorporates their multi-result summaries
- Any low-confidence findings pause the pipeline for user confirmation
- Pipeline completes successfully even if one domain agent instance fails
- Analysis status endpoint returns domain_results_summary with per-group execution details
- Pipeline still works when orchestrator_output is None (triage-only flow preserved)
- SSE pre-emission uses compute_agent_tasks (single source of truth, no duplicated logic)
</success_criteria>
