...
<task type="auto">
  ...
  <verify>
    Run the following single-file type check to verify the types file itself has no internal errors:
    ```
    cd frontend && bunx tsc --noEmit src/types/knowledge-graph.ts 2>&1 | head -20
    ```
    If this shows only external import errors (from files not yet updated that import old types), that is acceptable — those files are rewritten in Plans 03-05. The types file itself must have zero internal errors (no undefined references, no `any` types, correct d3 type extensions).
  </verify>
  <done>
