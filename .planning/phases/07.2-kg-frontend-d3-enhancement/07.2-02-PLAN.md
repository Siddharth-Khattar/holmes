...
<task type="auto">
  <name>Task 1: Install media dependencies, create SourceViewerModal shell and PdfViewer</name>
  <files>
    frontend/src/components/source-viewer/SourceViewerModal.tsx
    frontend/src/components/source-viewer/PdfViewer.tsx
  </files>
  ...
  <verify>
    1. Both component files exist in `frontend/src/components/source-viewer/`
    2. `cd frontend && bun run typecheck` — SourceViewerModal and PdfViewer have no type errors
    3. No `any` types in either file
    4. Both have ABOUTME comments and clear props interfaces
    5. All 8 media dependencies installed (check package.json)
  </verify>
  <done>
    SourceViewerModal shell created with content type routing. PdfViewer created with @react-pdf-viewer, jump-to-page, and search/highlight. SourceViewerContent type defined. All media dependencies installed.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create AudioViewer, VideoViewer, and ImageViewer components</name>
  <files>
    frontend/src/components/source-viewer/AudioViewer.tsx
    frontend/src/components/source-viewer/VideoViewer.tsx
    frontend/src/components/source-viewer/ImageViewer.tsx
  ...
  <verify>
    1. All 3 component files exist in `frontend/src/components/source-viewer/`
    2. `cd frontend && bun run typecheck` — all three components have no type errors
    3. No `any` types in any file
    4. Each viewer component has clear props interface and ABOUTME comment
  </verify>
  <done>
    AudioViewer uses wavesurfer.js with waveform and transcript cards. VideoViewer uses HTML5 video with timestamp markers. ImageViewer provides zoom/pan. All follow Holmes design system.
  </done>
</task>

<task type="auto">
  <name>Task 3: Refactor evidence-source-panel to use shared source viewer</name>
  <files>
    frontend/src/components/app/evidence-source-panel.tsx
  </files>
  <action>
    ...
    5. Since the old `Evidence` type from `@/types/knowledge-graph.ts` was removed in Plan 01, define a minimal local compatibility type if needed by existing consumers, or update imports to use the new types. The key is that this file compiles.
