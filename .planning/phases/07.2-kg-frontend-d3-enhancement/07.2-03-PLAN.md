...
must_haves:
  ...
  artifacts:
    - path: "frontend/src/components/knowledge-graph/GraphSvg.tsx"
      provides: "D3 force-directed graph SVG canvas with nodes, edges, labels, zoom, drag"
      min_lines: 150
    ...
    - path: "frontend/src/hooks/useGraphSelection.ts"
      provides: "Selection state and DOM highlighting without re-running simulation"
...
Output: `GraphSvg.tsx` component rendering the force-directed graph, `useGraphSimulation.ts` hook managing D3 simulation lifecycle, `useGraphSelection.ts` hook managing selection highlighting.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    **B) Create `frontend/src/hooks/useGraphSelection.ts`**
    ...
    ABOUTME comment. This hook manages selection state and applies visual highlighting via D3 selection refs — completely SEPARATE from the simulation lifecycle.
    ...
    **CRITICAL PERFORMANCE PATTERN (from RESEARCH.md Pitfall 2):**
    The selection useEffect must NOT be in the same effect as simulation setup. It depends ONLY on `selectedEntityId` (and the ref objects, which are stable). When the user clicks a node, ONLY this effect runs — the simulation continues undisturbed.

    5. **Click handler setup:** The hook should expose `selectEntity` which the GraphSvg component calls when a node's `<circle>` is clicked. The click handler is attached in GraphSvg (not in this hook) because the node elements are created in the simulation hook.
  </action>
  <verify>
    ...
    3. `useGraphSelection` has a useEffect that depends only on `selectedEntityId`:
       ```
       grep -A1 "useEffect" frontend/src/hooks/useGraphSelection.ts
       ```
       Should show `[selectedEntityId]` in dependency array.
    4. No `any` types in either file
    5. Both files have ABOUTME comments
  </verify>
  <done>
    `useGraphSimulation` manages D3 force simulation with 5 forces, D3 ref-based tick updates (zero React re-renders), sqrt-scaled node radius, radial centrality force, edge deduplication, drag/zoom. `useGraphSelection` manages selection highlighting in a separate effect that does not restart simulation. Both hooks have correct TypeScript types with no `any`.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    **Implementation:**
    ...
    4. Call `useGraphSelection({ nodeGroupRef, linkGroupRef, labelGroupRef, forceNodes, forceLinks })` to get selection state
  ...
  <verify>
    ...
    4. The file has ABOUTME comment
    5. No `any` types
  </verify>
  <done>
    `GraphSvg.tsx` renders the D3 force-directed graph with dark background, dot pattern, zoom controls, instruction bar, node/edge tooltips, and edge deduplication display. All dynamic rendering is via D3 refs. Selection propagates to parent via callback. Background click deselects.
  </done>
</task>
...
- `useGraphSelection` effect depends ONLY on selectedEntityId (does not restart simulation)
...
<success_criteria>
The core D3 graph visualization is complete: force simulation with 5 forces, sqrt-scaled node sizing, radial centrality layout, edge deduplication, selection highlighting, drag/zoom/pan — all using D3 refs for performance (zero React re-renders during simulation). The GraphSvg component receives data and emits selection events to its parent.
</success_criteria>
