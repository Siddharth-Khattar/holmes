...
<task type="auto">
  ...
  <action>
    ...
    **Expanded state (on click):**
    ...
    - Below: Source document card (if `source_finding_ids` exist):
      ...
      - "View source" link/button — calls `onViewSource` with appropriate `SourceViewerContent` derived from the relationship's source data
        - NOTE: We may not have a direct file URL from the relationship. The `source_finding_ids` point to case_findings, which link to agent_executions, which link to files. For Phase 7.2, the "View source" button can:
          1. If the relationship has `evidence_excerpt`, open a text display in the source viewer
          2. If we can resolve a file URL (via a separate API call or embedded data), open the appropriate viewer
          3. If neither, show "Source not available" gracefully
        - This is noted as an Open Question in RESEARCH.md. For this plan, wire the callback and pass what data is available. The actual file URL resolution may need a small enhancement in Plan 05 or a follow-up.
  ...
  <verify>
    ...
    4. Filter input in timeline sidebar works (filters by connected entity name)
    5. No `any` types
    6. Both files have ABOUTME comments
  </verify>
  <done>
    EntityTimeline sidebar shows chronological relationships for selected entity with header (name, type, date range, count), filter input, and scrollable timeline. EntityTimelineEntry is expandable, showing color-coded relationship descriptions and source document cards with "View source" links. Styled consistently with Holmes design system and Command Center sidebar patterns.
  </done>
</task>
...
- onViewSource callback is wired (actual file URL resolution may be partial)
...
<success_criteria>
Both panel components are complete: FilterPanel enables investigative filtering by domain, entity type, keywords, and search highlighting. EntityTimeline shows a rich chronological view of relationships with expandable source cards. Both are ready to be wired into the 3-panel layout in Plan 05.
</success_criteria>
