...
must_haves:
  truths:
    - "InvestigationTask model exists with all required columns"
    - "Case model has verdict_label and verdict_summary columns"
    - "Alembic migration creates investigation_tasks table and adds Case columns"
    - "SynthesisOutput Pydantic schema captures all Synthesis Agent output categories"
    - "API response schemas exist for every synthesis data type"
  artifacts:
