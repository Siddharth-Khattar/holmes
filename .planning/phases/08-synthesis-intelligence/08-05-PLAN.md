...
<objective>
Wire the Timeline page to real synthesis-generated API data and add verdict badge to case header and case list.

Purpose: Timeline currently uses mock data with API fallback. After synthesis runs, real timeline events exist in the DB. The case header and list also need verdict badges to show synthesis results at a glance.

Output: Timeline fetches from real API (mock removed), updated filter dimensions, verdict badge in case header and list.
</objective>
...
<task type="auto">
  <name>Task 2: Verdict badge in case header and case list</name>
  <files>
    frontend/src/app/(app)/cases/[id]/layout.tsx
    frontend/src/components/app/case-nav-section.tsx
    frontend/src/lib/api/cases.ts
  ...
  <action>
**1. Update case types** (check `frontend/src/types/case.ts` or wherever the Case interface is defined):
...
**2. Update case header in `frontend/src/app/(app)/cases/[id]/layout.tsx`**:
...
**4. If the case API client** (`frontend/src/lib/api/cases.ts`) has a fetch function that returns Case objects, verify it passes through the new `verdict_label` and `verdict_summary` fields from the backend response. No code change should be needed if it's using spread or generic JSON parsing, but verify.

**5. Backend case schemas** -- check `backend/app/schemas/case.py` to see if CaseResponse schema needs `verdict_label` and `verdict_summary` fields added. If the schema explicitly lists fields (not using `from_attributes` passthrough), add:
```python
verdict_label: str | None = None
verdict_summary: str | None = None
```

This is necessary for the backend to include these fields in API responses.
  </action>
  <verify>Run `cd /Users/siddharth/Development/Hackathons/Gemini_3_2025/holmes/frontend && bun run typecheck && bun run build` -- both pass. Check that the case layout file references verdict_label.</verify>
  <done>Case header shows verdict badge (Conclusive=green, Substantial=amber, Inconclusive=red) when synthesis complete, shows "Pending Analysis" otherwise. Case list shows verdict badge per case. verdict_summary displayed as subtitle in case header. Backend CaseResponse schema includes verdict_label and verdict_summary fields.</done>
</task>
...
- Backend CaseResponse schema includes verdict fields
</verification>
