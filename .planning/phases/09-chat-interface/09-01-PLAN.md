...
<task type="auto">
  ...
  **Tool 2: `make_get_findings_tool(case_id)`**
  Returns `async def get_findings(agent_type: str | None = None, category: str | None = None, min_confidence: float = 0.0, limit: int = 50) -> dict`.
  - Query `CaseFinding` where `case_id` matches
  - Optional filters: agent_type, category, confidence >= min_confidence
  - Order by confidence DESC, limit capped at 100
  - Return `{"findings": [...], "count": N}`
  - Finding fields: id (str), agent_type, category, title, finding_text (truncated to first 500 chars), confidence, citations (the JSONB array as-is)
  ...
  **Tool 4: `make_search_findings_tool(case_id)`**
  ...
  4. Define tool usage strategy: "Try these tools in order: (1) get_synthesis for summary-level questions, (2) query_knowledge_graph for entity/relationship questions, (3) get_findings for domain-specific details, (4) search_findings for keyword-specific queries."
  ...
  <done>
  4 tool factory functions compile and type-check. System prompt builder produces a string with case context injected. All tool functions have full docstrings and type hints.
  </done>
</task>
...
<task type="auto">
  ...
  **`create_chat_agent_and_runner(case_id: str, user_id: str, context: dict) -> tuple[Runner, Session]`**
  ...
  - Create LlmAgent directly (NOT via AgentFactory -- chat agent has different config):
    - `name=_safe_name("chat", case_id)` (import `_safe_name` from factory.py or inline the logic)
    ...
    - `generate_content_config=types.GenerateContentConfig(temperature=0.3)` for factual grounding
  - Create Runner via `create_stage_runner(agent)`
  - Create or get session via `get_or_create_stage_session()` with a deterministic session ID based on `case_id + user_id + "chat"` (so conversation persists within a session, but clear button creates new session by generating a new workflow_id)
...
5. System prompt includes case context injection and citation format instructions
6. SSE events follow the defined format: chat-token, chat-tool-start, chat-tool-end, chat-done, chat-error
</verification>
