...
must_haves:
  ...
  artifacts:
    ...
    - path: "frontend/src/types/detail-sidebar.ts"
      provides: "onViewFinding callback added to KnowledgeGraphEntityContent.props"
    - path: "frontend/src/components/app/detail-sidebar.tsx"
      provides: "Passes onViewFinding through to KnowledgeGraphEntityPanel"
  key_links:
...
Output: 6 modified files with working citation click-to-source and entity name resolution
</objective>
...
<task type="auto">
  ...
  <action>
**Step 1: Update detail-sidebar types for KG entity onViewFinding callback**

In `frontend/src/types/detail-sidebar.ts`:
- Add `onViewFinding?: (findingId: string) => void` to `KnowledgeGraphEntityContent.props`

In `frontend/src/components/app/detail-sidebar.tsx`:
- For the `knowledge-graph-entity` case in `renderContent`, pass `onViewFinding={descriptor.props.onViewFinding}` to `KnowledgeGraphEntityPanel`

**Step 2: KnowledgeGraphEntityPanel.tsx -- Source Documents section:**
...
**Step 3: EntityTimelineEntry.tsx -- "View source" link:**
...
**Step 4: KnowledgeGraphEntityPanel.tsx -- pass onViewSource through to EntityTimelineEntry:**
...
**Step 5: KnowledgeGraphCanvas.tsx -- wire useSourceNavigation and pass callbacks:**
...
3. In the `handleEntitySelect` callback (or wherever the detail sidebar content is set for KG entity), pass `onViewFinding={openFromFinding}` to `KnowledgeGraphEntityPanel` via the sidebar descriptor. When calling `setContent({ type: "knowledge-graph-entity", props: { ..., onViewFinding: openFromFinding } })`, pass the callback.
...
4. Merge the nav source content with the existing KG source viewer state. If `navSourceContent` is set, it takes priority over the existing `sourceViewerContent` state. Use a combined approach:
  ...
  <done>
    - detail-sidebar.ts has onViewFinding in KnowledgeGraphEntityContent.props
    - detail-sidebar.tsx passes onViewFinding through to KnowledgeGraphEntityPanel
