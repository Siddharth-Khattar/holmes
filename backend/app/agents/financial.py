...
def parse_financial_output(events: list[Event]) -> FinancialOutput | None:
    """Parse FinancialOutput from ADK events.
    ...
    Returns:
        Parsed FinancialOutput, or None if parsing fails.
    """
            ...
            for part in event.content.parts:
                if part.text:
                    ...
                    try:
                        ...
                        logger.warning(
                            "Failed to parse financial output JSON: %s",
                            exc,
                        )
                        continue

    logger.error("No valid financial output found in agent events")
    return None
...
async def _prepare_financial_content(
    files: list[CaseFile],
    gcs_bucket: str,
    hypotheses: list[dict[str, object]],
    context_injection: str | None = None,
) -> types.Content:
    """Build multimodal content for Financial Agent.
...
async def run_financial(
    ...
    Creates a FRESH ADK session for the financial stage (stage-isolated).
    ...
    Returns:
        FinancialOutput with findings and entities, or None on failure.
    """
    ...
    try:
        ...
        content = await _prepare_financial_content(
            files, settings.gcs_bucket or "", hypotheses, context_injection
        )

        # ---- Attempt 1: Pro model ----
        financial_output: FinancialOutput | None = None
        ...
        for attempt in range(1 + MAX_PARSE_RETRIES):
            ...
            stage = f"financial{stage_suffix}"
            if attempt > 0:
                stage = f"financial{stage_suffix}_retry_{attempt}"
            ...
            financial_output = parse_financial_output(attempt_events)
            if financial_output is not None:
                break

            if attempt < MAX_PARSE_RETRIES:
                logger.warning(
                    "Financial parse failed on attempt %d/%d for case=%s, "
        ...
        if financial_output is None:
            logger.warning(
                "Financial Pro model failed for case=%s, falling back to Flash",
                case_id,
            )
            fallback_used = True
            ...
            for attempt in range(1 + MAX_PARSE_RETRIES):
                ...
                stage = f"financial{stage_suffix}_fallback"
                if attempt > 0:
                    stage = f"financial{stage_suffix}_fallback_retry_{attempt}"
                session = await get_or_create_stage_session(
                    user_id=user_id,
                    case_id=UUID(case_id),
                    workflow_id=UUID(workflow_id),
                ...
                if attempt < MAX_PARSE_RETRIES:
                    logger.warning(
                        "Financial Flash fallback parse failed on attempt %d/%d "
                        "for case=%s, retrying...",
                        attempt + 1,
                        1 + MAX_PARSE_RETRIES,
                        case_id,
        ...
        if financial_output:
            output_data_record = financial_output.model_dump(mode="json")
        ...
        logger.info(
            "Financial completed case=%s workflow=%s execution=%s status=%s "
        ...
        logger.exception(
            "Financial failed for case=%s workflow=%s: %s",
            case_id,
            workflow_id,
            exc,
        )
