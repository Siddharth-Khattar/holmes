...
def parse_legal_output(events: list[Event]) -> LegalOutput | None:
    """Parse LegalOutput from ADK events.
    ...
    Returns:
        Parsed LegalOutput, or None if parsing fails.
    """
            ...
            for part in event.content.parts:
                if part.text:
                    ...
                    try:
                        ...
                        return LegalOutput.model_validate(data)
                    except (json.JSONDecodeError, ValueError) as exc:
                        logger.warning(
                            "Failed to parse legal output JSON: %s",
                            exc,
                        )
                        continue

    logger.error("No valid legal output found in agent events")
    return None
...
async def _prepare_legal_content(
    files: list[CaseFile],
    gcs_bucket: str,
    hypotheses: list[dict[str, object]],
    context_injection: str | None = None,
) -> types.Content:
    """Build multimodal content for Legal Agent.
...
async def run_legal(
    ...
    Creates a FRESH ADK session for the legal stage (stage-isolated).
    ...
    Returns:
        LegalOutput with findings and entities, or None on failure.
    """
    ...
    try:
        ...
        content = await _prepare_legal_content(
            files, settings.gcs_bucket or "", hypotheses, context_injection
        )

        # ---- Attempt 1: Pro model ----
        legal_output: LegalOutput | None = None
        ...
        for attempt in range(1 + MAX_PARSE_RETRIES):
            ...
            stage = f"legal{stage_suffix}"
            if attempt > 0:
                stage = f"legal{stage_suffix}_retry_{attempt}"
            ...
            legal_output = parse_legal_output(attempt_events)
            if legal_output is not None:
                break

            if attempt < MAX_PARSE_RETRIES:
                logger.warning(
                    "Legal parse failed on attempt %d/%d for case=%s, "
        ...
        if legal_output is None:
            logger.warning(
                "Legal Pro model failed for case=%s, falling back to Flash",
                case_id,
            )
            fallback_used = True
            ...
            for attempt in range(1 + MAX_PARSE_RETRIES):
                ...
                stage = f"legal{stage_suffix}_fallback"
                if attempt > 0:
                    stage = f"legal{stage_suffix}_fallback_retry_{attempt}"
                session = await get_or_create_stage_session(
                    user_id=user_id,
                    case_id=UUID(case_id),
                    workflow_id=UUID(workflow_id),
                ...
                if attempt < MAX_PARSE_RETRIES:
                    logger.warning(
                        "Legal Flash fallback parse failed on attempt %d/%d "
                        "for case=%s, retrying...",
                        attempt + 1,
                        1 + MAX_PARSE_RETRIES,
                        case_id,
        ...
        if legal_output:
            output_data_record = legal_output.model_dump(mode="json")
        ...
        logger.info(
            "Legal completed case=%s workflow=%s execution=%s status=%s "
        ...
        logger.exception(
            "Legal failed for case=%s workflow=%s: %s",
            case_id,
            workflow_id,
            exc,
        )
