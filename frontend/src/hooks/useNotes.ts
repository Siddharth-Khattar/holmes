...
interface UseNotesReturn {
    ...
    createAudio: (file: File | Blob, duration?: number) => Promise<NoteResponse | null>;
    update: (noteId: string, data: { content?: string; title?: string; subtitle?: string }) => Promise<NoteResponse | null>;
...
export function useNotes(caseId: string): UseNotesReturn {
                    ...
                    prev.map((n) =>
                        n.id === noteId
                            ? {
                                ...
                                content: result.content !== undefined ? result.content : n.content,
                            }
                            : n
                    )
                );
    ...
    const createAudio = useCallback(
        async (file: File | Blob, duration?: number): Promise<NoteResponse | null> => {
            try {
    ...
    const exportAsEvidence = useCallback(
        async (noteId: string, description?: string): Promise<boolean> => {
            try {
                ...
                setNotes((prev) =>
                    prev.map((n) =>
                        n.id === noteId ? { ...n, is_exported: true } : n
                    )
                );
                return true;
            } catch (err) {
