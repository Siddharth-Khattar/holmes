...
export async function listNotes(
    ...
    if (!res.ok) {
        const error = await res.json().catch(() => ({ detail: "Failed to list notes" }));
 ...
 */
export async function getNote(caseId: string, noteId: string): Promise<NoteResponse> {
    const headers = await getAuthHeaders();
    const res = await fetch(`${API_URL}/api/cases/${caseId}/notes/${noteId}`, { headers });
    if (!res.ok) {
        const error = await res.json().catch(() => ({ detail: "Failed to get note" }));
...
export async function createAudioNote(
    ...
    if (!res.ok) {
        const error = await res.json().catch(() => ({ detail: "Failed to upload audio" }));
...
export async function updateNote(
    ...
    if (!res.ok) {
        const error = await res.json().catch(() => ({ detail: "Failed to update note" }));
...
export async function deleteNote(caseId: string, noteId: string): Promise<void> {
    ...
    if (!res.ok) {
        const error = await res.json().catch(() => ({ detail: "Failed to delete note" }));
...
export async function getAudioUrl(
    ...
    if (!res.ok) {
        const error = await res.json().catch(() => ({ detail: "Failed to get audio URL" }));
...
export async function generateMetadata(
    ...
    if (!res.ok) {
        const error = await res.json().catch(() => ({ detail: "Failed to generate metadata" }));
...
export async function exportNoteAsEvidence(
    ...
    const headers = await getAuthHeaders();
    const res = await fetch(`${API_URL}/api/cases/${caseId}/notes/${noteId}/export`, {
        method: "POST",
        headers: {
            ...headers,
            "Content-Type": "application/json",
    ...
    if (!res.ok) {
        const error = await res.json().catch(() => ({ detail: "Failed to export note" }));
